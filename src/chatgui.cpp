...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))
{
    ...
    // send user text to chatbot 
     _panelDialog->sendMessageToChatlogic(std::string(userText.mb_str()));
}
...
void ChatBotPanelDialog::sendMessageToChatlogic(std::string text) { _chatLogic->SendMessageToChatbot(text); }

wxBitmap *ChatBotPanelDialog::getImageFromChatbot() {return _chatLogic->GetImageFromChatbot();}
